...
describe('Subscribers Module', () => {
  ...
  describe('Successfully handle subscribers', () => {
    ...
    it('Should edit the user', () => {
      assert.isFulfilled(subscribers.edit(id, sid, {reference: "new_reference"}))
    })
    ...
    it('Should reschedule the user', () => {
      assert.isFulfilled(subscribers.reschedule(id, sid, {startDate: {day: 10, month: 12}}))
    })
  ...
  describe('Error in subscription module functions', () => {
    it('Expect error in id type argument', () => {
      assert.throws(subscribers.activate.bind(subscribers, 300, sid), "Wrong type argument. 300 must be string, is number")
    })
  ...
  describe('Error in Subscriber Schema', () => {
    it('Expect validation error missing argument', () => {
      assert.isRejected(subscribers.create(id, {
        startDate: {
          day: 15,
          month: 5
        },
