...
requestManager.create = function(options) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    let headers
    if (options.private) {
      headers = requestManager.setHeaders('private')
    }
    else if (options.public) {
      headers = requestManager.setHeaders('public')
    }
    else {
      headers = requestManager.setHeaders()
    }
...
requestManager.setHeaders = function () {
  ...
  else if (arguments[0] === 'public') {
    if (!configurations.getPublicKey()) {
      error = new Error('Must set Public Key')
      throw error
    }
    else {
      headers = {
        'api-key': configurations.getPublicKey(),
        'Content-Type': requestManager.JSON_MIME_TYPE
      }
      return headers
    }
